...
export default class Assetlist extends PureComponent{
    ...
    addShopCart(){
        if(!this.props.isLogin){
            this.loginModel.setState({visible:true})
        }
    }
            ...
            <div className='list'>
                ...
                <div className="down">
                    ...
                    <div className='bottom'>
                        <Button onClick={()=>{this.buyButtonClick()}}>购买</Button>
                        {/* <span>购买</span> */}
                        <Link to="/profile/shopcart"><Icon onClick={()=>{this.addShopCart()}} type="shopping-cart" style={{fontSize:30,color:'black'}}/></Link>
                    </div>
                </div>
            </div>
        )
